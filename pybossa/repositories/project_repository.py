...
class ProjectRepository(Repository):
    ...
    def get(self, id):
        current_app.logger.warn('uamaxua Project id get = {}'.format(id))
        return self.db.session.query(Project).get(id)

    def get_by_shortname(self, short_name):
        current_app.logger.warn('uamaxua Project short name = {}'.format(short_name))
        return self.db.session.query(Project).filter_by(short_name=short_name).first()

    def get_by(self, **attributes):
        current_app.logger.warn('uamaxua Project attributes = {}'.format(attributes))
        return self.db.session.query(Project).filter_by(**attributes).first()

    def get_all(self):
        current_app.logger.warn('uamaxua Project get all')
    ...
    def filter_by(self, limit=None, offset=0, yielded=False, last_id=None,
        ...
        current_app.logger.warn('uamaxua Project filters: {}, {}, {}, {}, {}, {}, {}'.format(limit, offset, yielded, last_id, fulltextsearch, desc, filters))
